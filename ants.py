...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if not (isinstance(insect, QueenAnt) and len(insect.copy) <= 1):
            if insect.is_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        d = 0
        pcopy = self.place
        while not pcopy.bees or d < self.min_range or d > self.max_range:
            if pcopy == hive:
                return None
            d += 1
            pcopy = pcopy.entrance
        if pcopy == hive:
            return None

        elif d >= self.min_range or d <= self.max_range:
            return random_or_none(pcopy.bees)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.impostor.append(1)
        self.copy = self.impostor[:]
    ...
    def action(self, colony):
        ...
        if len(self.copy) >= 2:
            ...
            while p.exit != None:
                p = p.exit
                if p.ant != None:
                    if p.ant.container and p.ant.ant != None and not p.ant.ant in self.double_ants:
                        p.ant.ant.damage *= 2
                        self.double_ants.append(p.ant.ant)
                    if not p.ant in self.double_ants:
                        p.ant.damage *= 2
                        self.double_ants.append(p.ant)
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    time = 0
    uneffected_action = bee.action
    def new_action(colony):
        nonlocal time
        if time < duration:
            time += 1
            return effect(uneffected_action)(bee, colony)
        else:
            return uneffected_action(colony)
